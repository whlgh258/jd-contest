 ...
 * 2012-11-27
 */
public class MultiThreadsControllerPrediction
{
	private static final Logger log = Logger.getLogger( MultiThreadsControllerPrediction.class );
...
//            BlockingQueue<Map<String, Object>> testRows = new ArrayBlockingQueue<Map<String, Object>>(1455980);
			...
			//start producer
			UserProducerPrediction producer = new UserProducerPrediction(userQueue, limits);
			...
			{
				threads[i] = new Thread(new UserConsumerPrediction(userQueue, testRows, connections[i]));
				threads[i].start();
			}
            ...
            log.info("prediction: " + testRows.size());
	...
	{
		MultiThreadsControllerPrediction controller = new MultiThreadsControllerPrediction();
		controller.control();
	}
    ...
    private void writeToFile(Set<Map<String, Object>> set){
        ...
        try {
            CSVWriter writer = new CSVWriter( new FileWriter("/home/wanghl/jd_contest/0417/prediction.csv"), ',', '\0');
